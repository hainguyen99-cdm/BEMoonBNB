...
(function () {
    ...
    function i(e) {
        // console.log("function i g[e] e=" + e, g[e])
        return g[e]
    }
    let w = 0
        , m = null;
    ...
    function A(e, n) {
        ...
        // console.log( E.decode(S().subarray(e, e + n)))
    }
    function c(e) {
...
${e.stack}` : r
    ...
    function D(e) {
        try {
            ...
            if (r)
                throw l(t);
            // console.log("function D l(n)",l(n))
            return l(n)
        } finally {
    ...
    async function R(e) {
        ...
        if (typeof e < "u" && Object.getPrototypeOf(e) === Object.prototype) {
            ({ module_or_path: e } = e);
        } else {
            console.warn("using deprecated parameters for the initialization function; pass a single object instead");
        }

        // Nếu e chưa được định nghĩa, gán giá trị là đường dẫn tới file local
        if (typeof e > "u") {
            // Sử dụng path.join để tạo đường dẫn tới file WebAssembly
            e = path.join(__dirname, "/game_wasm_bg-DYwJl-6R.wasm");
        }

        // Đọc file WebAssembly từ đường dẫn local
        const fileBuffer = fs.readFileSync(e); // Đọc file

        // Chuyển đổi fileBuffer sang ArrayBuffer
        const arrayBuffer = Uint8Array.from(fileBuffer).buffer; // Chuyển đổi Buffer sang ArrayBuffer

        // Tạo biến n từ hàm $
        const n = B();

        // Khởi tạo instance của WebAssembly
        const { instance: t, module: r } = await $(arrayBuffer, n); // Truyền arrayBuffer vào hàm D

        // Trả về giá trị sau khi khởi tạo
        return L(t, r);
    }
...
function uuid() {
    return "xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx".replace(/[xy]/g, function (ue) {
...
let BlumPayload = {
    u: async function (idGame) {
        // Gọi hàm R để load module
        await global.R({});
        let id = uuid()
        // console.log("id", id);
        let k = global.D(idGame)
        if (!k || !k.hash || !k.nonce) {
            console.error("Error: Missing hash or nonce in response from D(idGame)");
            return;
        }
        let hash = k.hash
        let nonce = k.nonce
        ...
        const CLOVER = getRandomInt(200, 280);
        const FREEZE = getRandomInt(5, 10);
        const TRUMP = getRandomInt(15, 30);
        const HARRIS = getRandomInt(15, 30);
        const total = CLOVER + TRUMP * 5 + HARRIS * 5

        // console.log("amount", total);
        // await sleep(3000)
        // console.log("=============================");

        // Sau khi module được load thành công, gọi hàm B
        //     global.g[133].push({
        //     "id": id,
        //     "nonce": nonce,
        //     "hash": hash
        // })
        let i =  global.F(idGame,
            {
                "id": id,
                "nonce": nonce,
                "hash": hash
