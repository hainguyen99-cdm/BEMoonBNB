...
${e.stack}` : r
    ...
    async function D(e, n) {
        if (typeof Response == "function" && e instanceof Response) {
            ...
            // Kiểm tra xem e có phải là một Buffer source không
            if (!(e instanceof ArrayBuffer)) {
                throw new TypeError("Argument 0 must be a buffer source");
            }
    ...
    async function R(e) {
        ...
        if (typeof e > "u") {
            ...
            e = path.join(__dirname, "game_wasm_bg-BnV071fP.wasm");
        }
        ...
        const { instance: t, module: r } = await D(arrayBuffer, n); // Truyền arrayBuffer vào hàm D
...
let BlumPayload = {
    ...
    let id=uuid()
    console.log(id);
    let k = global.F(idGame)
    let hash = k.hash
    let nonce = k.nonce
    console.log(hash)
    console.log(nonce)
    const randomNum = getRandomInt(150, 280);
    // Sau khi module được load thành công, gọi hàm B
   
   let i = global.B(idGame, {
        "id": id,
        "nonce": nonce,
        "hash": hash
    }, {
